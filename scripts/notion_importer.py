...
book_notes_request: Dict = {"database_id": "5677c26a86304eb5b65eb8f4a02a0657",
                            # "page_size": 20,
                            "filter": {"property": "processed", "checkbox": {"equals": True, }, },
                            "sort": {"property": "last_edited_time", "timestamp": "last_edited_time",
...
def main():
    last_run = datetime.strptime(Path('last_run').read_text(encoding="utf-8").strip(), "%Y-%m-%d %H:%M:%S")
    book_pages: List[Dict] = get_database(book_notes_request)
    for book_page in book_pages:
        page_last_modified = datetime.strptime(book_page["last_edited_time"], "%Y-%m-%dT%H:%M:%S.%fZ")
        ...
        cover = book_page["icon"].get('external', {}).get('url', None) \
                or book_page["icon"].get('file', {}).get('url', None)
        cover_image_name = f"image.{cover.split('.')[-1].split('?')[0]}"
        post_text = hugo_file_head.substitute(
            post_title=title, date=read_date, title=book_title, author=author,
            publisher=publisher, publish_date=publish_date, pages=pages_count,
            read_date=read_date, image=cover_image_name) + '\n\n' + book_markdown
    ...
    Path('last_run').write_text(datetime.now().strftime("%Y-%m-%d %H:%M:%S"), encoding="utf-8")
